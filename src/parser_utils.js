...
let iface;
let { EXPRARG } = parser;
let { SCRIPTARG } = parser;
let { SUBSTARG } = parser;
let { LISTARG } = parser;
let cmd_parse_info = {
  if(words) {
    let special = [1, EXPRARG];
    let wordtext;
    let p;
    let last = last_real_word_number(words);
  ...
  subst(words) {
    return [last_real_word_number(words), SUBSTARG];
  },
  time: [1, SCRIPTARG],
  namespace(words) {
    let specials = [];
...
function tokname(type) {
  let name = parser.tokenname[type];
...
function list_elements(listtokens) {
  ...
  for (i = 0; i < listtokens.length; i++) {
    ...
    switch (token[0]) {
      ...
      default:
        throw new Error(
          `Unexpected list token: ${JSON.stringify(listtokens[i])}`,
        );
    }
  }
...
function list_words(listtokens) {
  // Convert to command-style list of words
  let i;
  let token;
  let word = [];
  let words = [];
...
function get_cmd_parse_info(params, cmd_text) {
  ...
  return params.get_cmd_parse_info == null ?
    null :
    params.get_cmd_parse_info(params, cmd_text);
}
...
function deep_parse(script_tok, params) {
  ...
  if (params.descend === undefined) {
    params.descend = function () {};
  }
  if (params.ascend === undefined) {
    params.ascend = function () {};
  }
  ...
  for (i = 0; i < commands.length; i++) {
    ...
    special = typeof parse_info === 'function' ? parse_info(command) : parse_info;
