...
var iface,
	...
	cmd_parse_info = {
		...
		'try': function (words) {
			var i = 2,
				specials = [],
				last = last_real_word_number(words);
...
function command_range(command) {
	var from = word_start(command[0]),
		to, i, j, word, token, type;
...
function deep_parse(script_tok, params) {
	var commands = script_tok[1],
		command, i, j, parse_info, special,
		cmd_text;
