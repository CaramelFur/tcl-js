...
subcmds = {
	...
	filter: function (c, args, I) {
		...
		function filter_key() {
			var regexes = [],
				i, j, keys = utils.objkeys(dictvals),
				key, re;
		...
		function filter_script() {
			var e, loopvars = args[0].GetList(),
				body = args[1],
				keyvar, valuevar, pairs = [],
				i = 0;
		...
		switch (filterType) {
			...
			case 'value':
				return filter_value();
			default:
				throw new TclError('bad filterType "' + filterType + '": must be key, script, or value', ['TCL', 'LOOKUP', 'INDEX', 'filterType', filterType]);
		}
	},
	...
	keys: function (c, args, I) {
		...
		var dictobj = args.shift(),
			glob = args.shift(),
			re, i, keys = utils.objkeys(dictobj.GetDict()),
			out;
	...
	values: function (c, args, I) {
		...
		var dictobj = args.shift(),
			dictval = dictobj.GetDict(),
			glob = args.shift(),
			re, e, i, keys = utils.objkeys(dictval),
			out;
