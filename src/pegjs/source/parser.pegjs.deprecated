...
// Simpleword
simpleWord
  = !quote !braceOpen parts:simpleWordPart+ { return new TclWord(parts); }
...
simpleWordPart
  ...
  )+ { return new TclWordPart(chars.join('')); }
  / bracketWordPart
  / variableWordPart

// Quotedword
quotedWord = quote parts:quotedWordPart* quote { return new TclWord(parts); }
...
// Wordparts
bracketWordPart
  = part:rawBracketWordPart {
      return new TclWordPart(part.slice(1, -1), TclWordPartTypes.bracket);
    }
...
variableWordPart
  = dollarSign
    vname:(
      chars:variableChar+ { return { name: chars.join(''), subname: null }; }
      / name:variableChar*
        parenthesisOpen
        subname:(!parenthesisClose any)*
        parenthesisClose { return { name, subname }; }
      / braceOpen name:(!braceClose any)* braceClose {
          return { name: name.join(''), subname: null };
        }
    ) { return new TclWordPart(vname, TclWordPartTypes.variable); }
