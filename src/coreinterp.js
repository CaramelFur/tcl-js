...
module.exports = function ( /* extensions... */ ) {
	var interp_args = Array.prototype.slice.call(arguments),
		I = this,
		mathops, mathfuncs, mathop_cache = [null, {}, {}];
	...
	function exec_command(args, c) {
		var cinfo = I.resolve_command(args[0]),
			result, needs_trampoline = false,
			asyncres, i = args.length;
	...
	this.compile_script = function (commands) {
		var i, j, wordf, command = [],
			out = [];

		function compile_word(tokens) {
			var i, token, word = [],
				async = false, array, expand = false,
		...
		function compile_command(command) {
			var i, word, expand = false,
				async = false, constant = true,
					f, dynwords = [];
	...
	function script_op(script) {
		...
		f = function (c) {
			return I.exec(script, function (res) {
				return c(res.result);
			});
		};
		f.async = true;
	...
	function mathfunc_op(parts) {
		var part, args = [],
			arg, i, j = 0,
			f, async = false, constant = true, constval,
	...
	function operator2func(name, args) {
		var takes = args.length,
			mathop = mathops[takes][name],
			cached, f, i,
			constant = true,
			async = false, outargs, dynargs, apply;
