...
function changecase(method) {
  return function (args, I) {
    ...
    let parts;
    let first = args[2] === undefined ? 0 : get_str_idx(str, args[2]);
    let last = args[3] === undefined ? str.length - 1 : get_str_idx(str, args[3]);
...
function int_range_check(bits) {
  return function (s) {
    ...
    if (
      ...
      if (Math.abs(value) >= Math.pow(2, bits)) {
        return null;
      }
...
subcmds = {
  ...
  first(args, I) {
    ...
    let idx = args[3] === undefined ? 0 : get_str_idx(haystack, args[3]);
  ...
  last(args, I) {
    ...
    let needle = args[1].toString();
    let haystack = args[2].toString();
    let idx = args[3] === undefined ? haystack.length : get_str_idx(haystack, args[3]);
    return new IntObj(haystack.lastIndexOf(needle, idx));
  },
  index(args, I) {
    I.checkArgs(args, 2, 'string index');
    let str = args[1].toString();
    let index = get_str_idx(str, args[2]);
    return new StringObj(str.charAt(index) || '');
  },
  range(args, I) {
    I.checkArgs(args, 3, 'string first last');
    let str = args[1].toString();
    let first = Math.max(0, get_str_idx(str, args[2]));
    let last = get_str_idx(str, args[3]);
  ...
  replace(args, I) {
    ...
    let str = args[1].toString();
    let first = Math.max(0, get_str_idx(str, args[2]));
    let last = Math.min(str.length, get_str_idx(str, args[3]));
    let parts = [];
    if (first > last) {
      return types.EmptyString;
    }
  ...
  wordstart(args, I) {
    ...
    let m;
    let str = args[1].toString();
    let idx = get_str_idx(str, args[2]);
    if (idx < 0) {
      return new IntObj(0);
    }
  ...
  compare: compare_strings((str1, str2) => {
  ...
  equal: compare_strings((str1, str2) => new BoolObj(str1 === str2)),
  is(args, I) {
